...
document.addEventListener("visibilitychange", () => {
    if (!quizState.active || quizState.banned || document.getElementById('quiz-active-area').classList.contains('hidden')) return;

    if (document.hidden) {
        quizState.leaveTime = Date.now();
        quizState.tabLeaves++;
    } else {
        const timeAway = Date.now() - quizState.leaveTime;
        if (timeAway > 5000 || quizState.tabLeaves >= 3) {
...
window.renderQuizQuestion = function() {
    ...
    q.options.forEach((opt, i) => {
        ...
        btn.className = 'answer-btn';
        btn.textContent = opt;
        btn.onclick = () => window.handleQuizAnswer(q, i);
        grid.appendChild(btn);
    });
...
window.endQuiz = function(isBanned) {
